...
from TMT_quant_0_0_1 import (ISOTOPE_OVERLAP, calc_isotope_envelopes,
                             calculate_P2T, calculate_S2I, fix_crosstalk,
                             get_intensities, interpolate_qc)
...
def test_fix_crosstalk():
    ...
    TMT_channels = np.array([
        93.51692447, 91.30369014,
        102.11652333, 98.39943001,
        102.9410515, 100.42391389,
        100.74452838, 98.28089182,
        101.90839719, 98.29796577,
        99.94893254
    ])
...
@pytest.mark.parametrize(
    ...
    [
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'da', 0.003, 2),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'da', 0.002, 2),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'da', 0.0031, 3),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'da', 0.0, 0),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'ppm', 20e-5, 3),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'ppm', 15.87396e-6, 2),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'ppm', 1e-5, 0),
        ([126], np.array([(126.002, 100), (126.003, 100), (125.998, 100)]), 'ppm', 0, 0),
    ],
    ids=[
...
def test_calc_isotope_envelopes(peptides, charges, expected_results):
    ...
    assert peptides == list(lib.keys())
    arr = np.array(lib['ELVISLIVES']['env'][(('N', '0.000'),)][charges[0]]['mz'])
    assert (abs(arr - expected_results[1]) < expected_results[1] * 5e-6).any()

@pytest.mark.parametrize(
    "ms1_spec,peptide,charge,window,result",
    [
        (Spectrum(peaks=np.array(
            [(1101.6401678803702, 100), (1102.6431931587235, 400), (1101.8401678803702, 500)]
        )),
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 0.5),
        (Spectrum(peaks=np.array(
            [(1101.6401678803702, 100), (1102.6431931587235, 400)]
        )),
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 1.0),
        (Spectrum(peaks=np.array(
            [(1101.8401678803702, 500)]
        )),
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 0.0),
    ],
    ids=[
...
def test_calculate_S2I(ms1_spec, peptide, charge, window, result):
    ...
    s2i = calculate_S2I(ms1_spec, peptide, charge, window, lib)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 500),
        (Spectrum(peaks=np.array(
            [(1101.6401678803702, 100), (1102.6431931587235, 400), (1101.8401678803702, 500)],
        ), noise_level=2),
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 250),
        (Spectrum(peaks=np.array(
            [(1101.6401678803702, 100), (1102.6431931587235, 400), (1101.8401678803702, 500)],
        ), noise_level=0.5),
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 1000),
        (Spectrum(peaks=np.array(
            [(1101.8401678803702, 500)],
        ), noise_level=1),
        'ELVISLIVES', 1, (1100.633+PROTON-2,1100.633+PROTON+2), 0.0),
...
def test_calculate_P2T(ms1_spec, peptide, charge, window, result):
    lib = lib = calc_isotope_envelopes([peptide], [charge])
    p2t = calculate_P2T(ms1_spec, peptide, charge, window, lib)
